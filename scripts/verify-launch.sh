...
if should_run ios && [ -z "${SKIP_IOS:-}" ]; then
  ...
  if [ $? -ne 0 ]; then
    ...
    if [ ! -d *.xcworkspace ] && ! ls *.xcworkspace >/dev/null 2>&1; then
      ...
      # Use xcodebuild's "generic" destination — builds for any iOS Simulator
      # without needing a specific UDID/name/OS. This is the canonical way to
      # build for Simulator when you don't need to run/test (which we don't —
      # we're verifying the build succeeds, that's it). Avoids every sim-
      # picker failure mode we hit with the UDID and name-based approaches.
      echo "  Building against: generic/platform=iOS Simulator"
